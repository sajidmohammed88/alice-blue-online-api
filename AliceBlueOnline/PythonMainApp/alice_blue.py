...
class AliceBlue:
    ...
    def __on_data_callback(self, ws=None, message=None, data_type=None, continue_flag=None):
        if(type(ws) is not websocket.WebSocketApp): # This workaround is to solve the websocket_client's compatiblity issue of
                                                    # older versions.
                                                                                                       # ie.0.40.0 which is used
                                                                                                       # in upstox.  Now this will
                                                                                                       # work in
                                                                                                       # both 0.40.0 & newer
                                                                                                       # version of
                                                                                                       # websocket_client
    ...
    def __api_call(self, url, http_method, data):
        #logger.debug('url:: %s http_method:: %s data:: %s headers:: %s', url,
        #http_method, data, headers)
